...
func main() {
	...
	id, minute := mostSleepIndex(guards)
	fmt.Printf("Guard #%d sleept %d times on %d minute\n", id, guards[id].sleepAmount[minute], minute)
	fmt.Printf("The answer is %d (%d*%d)", id*minute, id, minute)
}
...
func mostSleepIndex(guards map[int]guard) (guardID, minute int) {
	type maxPosition struct {
		id     int
		minute int
	}
	var (
		maxPos maxPosition = maxPosition{}
		maxVal int
	)

	for id := range guards {
		for minute := range guards[id].sleepAmount {
			if guards[id].sleepAmount[minute] > maxVal {
				maxVal = guards[id].sleepAmount[minute]
				maxPos.id = id
				maxPos.minute = minute
			}
		}
	}

	return maxPos.id, maxPos.minute
}
